class DeyeEnergyManagerCard extends HTMLElement {
  ...
  normalProfileInput(name, entityId, unit = "") {
    ...
    const step = Number(entity?.attributes?.step);
    const range = Number.isFinite(min) && Number.isFinite(max) && Number.isFinite(step) && step > 0;
    return `<label class="field"><input data-normal-profile-number="${this.escapeHtml(name)}" type="number" inputmode="decimal" value="${this.escapeHtml(value)}" ${range ? `min="${min}" max="${max}" step="${step}"` : ""} ${this.exists(entityId) && range ? "" : "disabled"}><span>${this.escapeHtml(unit)}</span></label>`;
  }
    ...
    this.innerHTML = `
      <ha-card class="theme-schedule-dark">
        <style>
          ...
          .dem-v074{padding:18px;display:grid;gap:16px;font-family:Roboto,Arial,sans-serif;font-size:14px}
