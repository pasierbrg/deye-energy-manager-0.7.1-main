...
- Karta: cache `v=0779`, klasa CSS `dem-v075`. W sekcji **Ustawienia normalnej pracy** zmieniono etykietę `SOC baterii Deye (TOU)` na `Minimalny SOC`; pole jest zawsze edytowalne dzięki zapasowym zakresom `min/max/step` i nigdy nie jest blokowane przez brak aktualnego stanu encji.
