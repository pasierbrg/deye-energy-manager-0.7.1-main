...
```text
/deye_energy_manager/deye-energy-manager-card.js?v=0771
```

Przy instalacji ręcznej użyj:

```text
/local/deye-energy-manager-card.js?v=0771
```
...
3. Zmień parametr cache zasobu na `v=0771`.
...
8. Po Stop Sell lub zatrzymaniu awaryjnym użyj **System i diagnostyka → Włącz Manager i harmonogram**. Przycisk włącza `Schedule` i Scheduler, lecz nie zmienia ustawienia **Ładowanie z sieci** slotów; tylko wartość `tak` zezwala na ładowanie z sieci.
...
Po ręcznym skopiowaniu nowej karty do `/config/www/` użyj zasobu `/local/deye-energy-manager-card.js?v=0771`, przeładuj zasoby Lovelace i wykonaj `Ctrl + F5`. Jeśli korzystasz z karty dostarczanej przez integrację, użyj adresu `/deye_energy_manager/deye-energy-manager-card.js?v=0771`.
...
Każdy slot ma jedno pole **Minimalny SOC sprzedaży**, używane wyłącznie jako warunek sprzedaży `Selling First`. Nie ma osobnego pola „SOC TOU Deye” w edytorze slotu. Slot `Charge` korzysta z globalnego profilu **Ustawienia ładowania**, gdzie **Docelowy SOC** jest fizyczną wartością przekazywaną do odpowiedniego zakresu Deye Time Of Use. Po zapisie falownik może opublikować nowy stan z opóźnieniem: integracja nasłuchuje zmian encji Deye i wykonuje odczyt kontrolny po 1, 2 i 4 sekundach, maksymalnie przez 90 sekund, bez ponownego wysyłania tej samej transakcji; w tym czasie diagnostyka pokazuje oczekiwanie, a nie błąd.

## Ustawienia ładowania

W **Ustawienia i diagnostyka → Ustawienia ładowania** zapisz wspólny profil dla aktywnych slotów `Charge`: prąd ładowania, prąd rozładowania, prąd ładowania z sieci oraz **Docelowy SOC**. W samym slocie wybierz **Ładowanie z sieci: tak**, aby zezwolić na Grid Charge. Przy wartości `nie` Grid Charge pozostaje wyłączone, a slot może ładować baterię wyłącznie z PV.
